...
from app1.models import ContactPln
...
def rechargeadded(request):
    if request.method=="POST":
        bname=request.POST.get("bname")
        Mno=request.POST.get("Mno")
        OPERATOR=request.POST.get("OPERATOR")
        PLAN=request.POST.get("PLAN")
        t=bname+Mno+OPERATOR+PLAN
        try:
            r=ContactPln(bname=bname, Mno=Mno,OPERATOR=OPERATOR,PLAN=PLAN)
            r.save()
            return render(request,"recharge.html",{"msg":"Recharge Done !!"})
        except:
            return render(request,"recharge.html",{"msg":"Recharge Already Done !!"})
        
        
def previousrecharge(request):
    r=ContactPln.objects.all()
    return render(request,'oldrecharge.html',{'data':r})
